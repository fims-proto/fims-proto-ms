...
type CalculateBalanceByLedgersCmd struct {
	Ids []uuid.UUID
}

type CalculateBalanceByPeriodCmd struct {
	PeriodId uuid.UUID
}
...
func (h CalculateLedgerBalanceHandler) HandleCalculateBalanceByPeriod(ctx context.Context, cmd CalculateBalanceByPeriodCmd) (err error) {
	log.Info(ctx, "handle calculating ledger balance by period, cmd: %+v", cmd)
	defer func() {
		if err != nil {
	...
	return h.repo.UpdatePeriodLedgers(ctx, cmd.PeriodId, func(ledgers []*domain.Ledger) ([]*domain.Ledger, error) {
		return h.calculateBalance(ctx, ledgers)
	})
}

func (h CalculateLedgerBalanceHandler) HandleCalculateBalanceByLedgers(ctx context.Context, cmd CalculateBalanceByLedgersCmd) (err error) {
	log.Info(ctx, "handle calculating ledger balance by ledgers, cmd: %+v", cmd)
	defer func() {
		if err != nil {
			log.Err(ctx, err, "handle calculating ledger balance failed")
		}
...
func (h CalculateLedgerBalanceHandler) calculateBalance(ctx context.Context, ledgers []*domain.Ledger) ([]*domain.Ledger, error) {
	if len(ledgers) == 0 {
